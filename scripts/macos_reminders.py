...
def main(argv):
    if sys.platform != 'darwin' or argv not in (['install'], ['uninstall']):
        print('Usage on macOS: python3 scripts/macos_reminders.py install|uninstall', file=sys.stderr)
        return 2
    ...
    launcher = project / 'predict.sh'
    if not launcher.exists():
        print('Cannot find predict.sh beside the installer.', file=sys.stderr)
        return 2
    location = Path.home() / '.config' / 'radio' / 'location.json'
    if not location.exists():
        print('Save your location with ./predict.sh --save-location before installing reminders.', file=sys.stderr)
        return 2
    ...
    config = {
        'Label': LABEL,
        'ProgramArguments': [str(launcher), '--watch', '--no-plot'],
